...
(function () {
  ...
  const PALETTE = {
    man: '#c94b4b', // MUJI Terracotta / Apple Crimson (砖红/绯红)
    machine: '#3a78c4', // Apple/Google Tech Blue (科技蓝/黛蓝)
    material: '#5b8c6e', // MUJI Sage Green (鼠尾草绿/竹绿)
    method: '#c98a3b', // MUJI Ochre / Cedar Yellow (木皮黄/琥珀)
    environment: '#7a669e', // Lavender Gray / Slate Indigo (薰衣草灰/靛紫)
    measurement: '#3b919e' // Precision Teal (松石绿/碧色)
  };
  ...
  // 本文件 wrapTextByWidth 已依赖 UIUtils.measureTextWidth，自包含理由不成立。
  ...
  // 两者逻辑等价（CJK/Latin 分支一致），仅迭代写法不同（for-of vs for-index）。
  ...
  function generateSVG(data, options) {
    ...
    const TITLE_H = compact ? 15 : 20; // 仅作为顶部边距，留出空间给主题内容
    ...
    ALL_CATS.forEach((catId) => {
      ...
      causes.forEach((cause, ci) => {
        // 1. 一级原因文本换行与尺寸（加大字号：causeFs = 13px）
    ...
    // 自适应调整三列的附着点 X 坐标，防止大骨/中骨水平重叠与左出界
    ...
    // 如果三列排布超出了原有的鱼头起点，则动态扩宽画布宽度 W，并将鱼头向右推
    ...
    if (!compact) {
      ...
      // compact：简化三角鱼尾（与非 compact 保持一致的 fill="none" 风格）
      s +=
        '<path d="M ' +
        tailX +
        ' ' +
  ...
  function focusSvgGroup(catId) {
    ...
    catCards.forEach((el) => {
      ...
      // Clear styles written by pre-unification versions before applying the shared focus state.
      el.style.removeProperty('box-shadow');
      el.style.removeProperty('border-color');
    });

    // 图形保留分类色；表单始终使用全局主色焦点，避免分类色泄漏到输入区。
