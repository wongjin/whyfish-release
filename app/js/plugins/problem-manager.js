...
/**
* Validate a fresh model response before permissive legacy normalization.
* Persisted v1 assessments still go through normalizeAssessment(), but a new
* incomplete/truncated response must never silently become an all-1 report.
*/
function validateAssessmentContract(raw) {
...
requiredObjects.forEach(([name, value]) => {
if (!value || typeof value !== 'object' || Array.isArray(value)) issues.push(name);
});
['clarity', 'measurability', 'scope', 'evidence', 'investigability'].forEach((key) => {
const item = input.quality?.[key] || (key === 'investigability' ? input.quality?.actionability : null);
if (!item || !Number.isFinite(Number(item.score)) || !_text(item.reason || item.note)) {
issues.push(`quality.${key}`);
}
});
['severity', 'urgency', 'impact'].forEach((key) => {
const item = input.priority?.[key];
if (!item || !Number.isFinite(Number(item.score)) || !_text(item.reason || item.note)) {
issues.push(`priority.${key}`);
}
});
if (!Array.isArray(input.problemDefinition?.facts)) issues.push('problemDefinition.facts');
if (!Array.isArray(input.problemDefinition?.missingInformation)) {
issues.push('problemDefinition.missingInformation');
}
if (!Array.isArray(input.nextActions)) issues.push('nextActions');
...
const contractIssues = validateAssessmentContract(assessment);
if (contractIssues.length) {
throw new Error(
`AI 返回的评估结构不完整（缺少 ${contractIssues.slice(0, 5).join('、')}），未保存本次结果，请重试`
);
}
...
await _persistAssessmentState();
try {
savedReport = await _saveAssessmentReport();
} catch (e) {
...
await _saveAssessmentAIResult(savedReport?.id || '');
window.SidebarUI?.completeTask?.({
title: 'AI 问题评估报告',
...
async function _persistAssessmentState() {
...
const details = getData();
const problem = typeof getProblemById === 'function' ? getProblemById(activePid) : null;
...
return await window.updateProblem(activePid, { details, snapshot, analyses });
}
...
async function _saveAssessmentAIResult(reportId = '') {
if (
!_assessment ||
typeof window.saveAIResult !== 'function' ||
...
const d = collectFormData();
const q = _assessment.quality;
const method =
{ '5-why': '5 Whys', fishbone: '鱼骨图', fta: '故障树', none: '暂不开始根因分析' }[
_assessment.recommendedAnalysis.primaryMethod
] || '待确认';
const summary = [
`综合质量评分：${q.overall.toFixed(1)}/5`,
`清晰度：${q.clarity.score}/5 — ${q.clarity.reason}`,
`可测量性：${q.measurability.score}/5 — ${q.measurability.reason}`,
`范围定义：${q.scope.score}/5 — ${q.scope.reason}`,
`证据支撑：${q.evidence.score}/5 — ${q.evidence.reason}`,
`可调查性：${q.investigability.score}/5 — ${q.investigability.reason}`,
`首选分析方法：${method}`,
..._assessment.nextActions.map(
(item) => `${item.priority}. ${item.action}${item.evidenceRequired ? `（证据：${item.evidenceRequired}）` : ''}`
)
].join('\n');
return await window.saveAIResult({
problemId,
tool: 'problem',
action: 'assessment',
title: `AI 问题评估报告 - ${d.title || _assessment.suggestedTitle || '未命名问题'}`,
status: 'success',
createdAt: new Date().toISOString(),
model: typeof getActiveModel === 'function' ? getActiveModel() : '',
reportId,
blocks: [{ kind: 'assessment', title: '评估结论', text: summary }]
});
}
...
clear: clearProblemForm,

fillFormData: fillForm,
...
validateAssessmentContract: validateAssessmentContract,
