...
function syncToolDom(problemId, methodKey) {
  ...
  if (methodKey === '5why' || methodKey === 'tool-5why') {
    ...
    // Fishbone's input handler collects the whole form into plugin state. Dispatch only
    // after every imported field is populated so toolpanel:show cannot restore a
    // title-only state over the phenomenon field during navigation.
