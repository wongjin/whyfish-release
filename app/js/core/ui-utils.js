...
function showToast(msg, type) {
  // Prevent duplicate toast when API key check has already triggered a specific warning toast.
...
function showConfirm(message, type = 'primary') {
  return new Promise((resolve) => {
    ...
    // 若存在打开的 native <dialog>，挂载到其内部以共享 top-layer 渲染上下文（避免被 dialog 遮挡）
    ...
    // L1: danger 类型默认聚焦取消按钮，避免用户误触发破坏性操作（WCAG 建议）
    if (type === 'danger') {
      cancelBtn.focus();
    } else {
      okBtn.focus();
...
function showConfirmWithOption(message, type = 'primary', checkboxLabel = '') {
  return new Promise((resolve) => {
    ...
    const btnClass =
      type === 'danger' ? 'btn-danger' : type === 'warning' ? 'btn-warning' : 'btn-primary';
    // M1: 使用唯一 ID 避免多实例时 checkbox 状态读取冲突
    ...
    const cancelBtn = dialog.querySelector('.confirm-cancel');
    const okBtn = dialog.querySelector('.confirm-ok');
    // M1: 通过 dialog 局部查询，不依赖全局 document.getElementById
...
function navigateTo(pageId, toolPanelId, keepMobileOpen) {
  ...
  // 切换页面后滚动回顶部。
  // .main-content 是实际滚动容器（overflow-y:auto），必须直接重置它的 scrollTop。
  // document.scrollingElement 和 window.scrollTo 均无效，因为
  // html/body/#app 都有 overflow:hidden，不会产生页面级滚动。
  // rAF 确保目标页面布局已完成（_switchPage 切换 .hidden 后高度才确定）。
...
// 转义所有原始 HTML（防止 XSS），同时保留 markdown 语法渲染
if (typeof marked !== 'undefined') {
  // Block dangerous URL schemes in markdown links and images
...
function safeSanitize(html) {
  ...
  // R5: DOMPurify 未加载时失败关闭（fail-closed），而非返回未消毒原文。
  // 退化为 HTML 转义，避免 AI / markdown 内容在 innerHTML 中造成 XSS。
  console.error(
...
function mdToHtml(md) {
  if (!md) return '';
  // P1-12: 守卫 marked 加载失败（CDN/script-tag 顺序问题），回退到转义文本
...
window.UIUtils = {
  ...
  downloadDocx: function (htmlContent, filename) {
    // M2: 确保传入内容已净化，防止未来调用方传入未净化 HTML 造成内容注入
  ...
  svgToPngBase64: function (svgElement) {
    return new Promise((resolve, reject) => {
      try {
        ...
        // SVG-to-Canvas parser and renderer helpers
