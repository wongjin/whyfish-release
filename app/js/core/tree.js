...
(function () {
  ...
  function renderTree() {
    ...
    // P1-7: canvas 可能在 renderTree 被调用时尚未挂载（插件初始化期、DOM 重排期）
    if (!canvas) return;
    if (!tree) {
      canvas.innerHTML = '';
      return;
  ...
  function _renderNodeCard(node, parentNode) {
    ...
    // Weight (only show if parent has multiple children)
    ...
    // Inline causal validation badge
    if (node.text && node.parentId !== null) {
  ...
  function renderNode(node, parentNode = null) {
    ...
    // Add cause button (unless it's a root cause)
  ...
  function addChild(parentId) {
    ...
    // P1-6: _renderNodeCard 期望第二参数是父节点（读 pn.text/parent.children.length），
    // 此前传的是祖父节点，导致 causal-hint 和 weight 控件用错误上下文渲染。
  ...
  function removeNode(nodeId) {
    ...
    // L1: clean up ghost Set entry to prevent collapsedNodeIds accumulating stale IDs
    collapsedNodeIds.delete(nodeId);
    ...
    if (parentId) {
      const parentNode = nodeIndex.get(parentId);
      // P1-6: 传父节点本身，而非祖父（见 addChild 同款修复）
      _patchNode(parentId, parentNode);
  ...
  function saveNodeText(nodeId, text) {
    if (window.updateTreeNodeText(nodeId, text)) {
      ...
      // P2-9: card 外部的"添加 Why N"按钮依赖 node.text 显隐，
      // _patchNode 只刷新 .why-node-card，需额外更新整个 node 的按钮状态。
      if (node) {
  ...
  function confirmNode(nodeId) {
    const input = document.querySelector(`textarea[data-node-id="${nodeId}"]`);
    if (input) {
      input.blur(); // triggers onblur → saveNodeText
    }
  }
  ...
  function openEvidenceModal(nodeId) {
    ...
    // P1-7: 元素可能未挂载，守卫后再访问 .value/.showModal()
  ...
  // Event delegation for tree canvas
  // Keep the target button mounted until click fires. Without this guard,
  // pointer focus changes blur the textarea, whose save handler redraws the
  // node and removes the pressed button before the click event can arrive.
  ...
  function handleTreeAction(e) {
    ...
    // M1: guard both null and NaN — Number('abc') returns NaN which !== null
  ...
  // Event delegation for textarea blur (save) and keydown
  ...
  // Event delegation for weight slider
