...
返回 JSON：
{
  "suggestions": [
    {
      "category": "维度",       // 设备/流程/材料/方法/环境/管理
      "text": "原因描述",
      "reasoning": "推荐理由"
    }
  ],
  "depthNote": "对当前分析深度的简要评估",
  "stopAdvice": true,   // true = 建议停止分析（已足够深入），false = 建议继续追问
  ...
  "clusters": [
    {
      ...
      "contribution": 35,            // 贡献百分比，所有聚类加总约100%
      "sourceIndices": [0, 3, 5],    // 原始根因索引
      "capa": "具体纠正措施"
    }
  ],
  "summary": "归并逻辑简述"
...
function renderPrompt(id, vars) {
  ...
  for (const [key, value] of Object.entries(vars)) {
    ...
    // 隔离：防止用户数据中的 {{ 干扰模板占位符解析
    const val = String(value ?? '').replace(/\{\{/g, '{ {');
    system = system.replaceAll(placeholder, val);
    user = user.replaceAll(placeholder, val);
  }
  // Clean up any remaining unrendered inputs to prevent confusing the LLM
  if (p.inputs) {
...
// 暴露模块 API 到全局（与其他 core 模块一致，便于测试与跨模块调用）
