...
window.AICtrl = {
  ...
  _inflightBtnIds: ['btnAISuggest', 'btnAutoAnalyze', 'btnVerifyCausality', 'btnConsolidate'],
...
const SidebarUI = {
  ...
  completeTask(options = {}) {
    ...
    task.settled = true;
    if (status !== 'error') this._persistTaskResult(task, status);
  },
...
async function getAISuggestions(nodeId, onReasoning) {
  ...
  const rendered = renderPrompt('suggest', {
    ...
    whyLevel: node ? node.level : 1,
    parentContext: node?.text ? `(上层原因: "${node.text}")` : ''
  });
  if (!rendered) {
