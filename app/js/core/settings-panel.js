...
document.getElementById('btnSaveSettings')?.addEventListener('click', async () => {
  try {
    ...
    // P1-3: 必须 await，否则紧接的 loadAllProviderConfigs 可能读到 IDB 旧数据
...
// Provider cards: consolidated event delegation
...
async function handleProviderCardClick(e) {
  try {
    ...
    // Card header click — switch provider
    if (header) {
    ...
    if (testBtn) {
      ...
      // P1-9: 测试期间禁用所有 provider 的测试按钮，防止并发测试 corrupt 共享 activeProviderId
