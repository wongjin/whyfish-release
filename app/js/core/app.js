...
if (window.__appInitialized) {
  ...
  const navigateToStep = function (step) {
    ...
    // Why N 对应 level N+1（根节点 level=1，第一个 Why level=2）
    const level = parseInt(step.replace('why', '')) + 1;
    highlightLevel(level);
  };

  // 焦点管理：将焦点移到当前可见区域的首个标题
  ...
  // ===== UI Event Handlers =====
  // (Settings panel → settings-panel.js, Report actions → report-actions.js)

  // 清空当前 5 Whys 分析。问题基本信息和报告库保持不变。
  document.getElementById('btnReset')?.addEventListener('click', async () => {
    if (
      ...
      return;
    // Capture before mutating state: clearing the active ID first would remove the
    // legacy base key instead of this problem's scoped 5 Whys key.
  ...
  // 5 Whys: import from problem pool
  document.getElementById('btnLoadFromProblem5Why')?.addEventListener('click', () => {
    openProblemPicker((problem) => {
      ...
      // Sync workspace UI visibility based on whether the loaded problem has an active tree with children
      if (
        window.Store.tree &&
  ...
  document.getElementById('btnStartAnalysis')?.addEventListener('click', async () => {
    ...
    // 如果树已经是这个问题的分析（根节点匹配且有子节点），直接进入工作区
  ...
  document.getElementById('btnAISuggest')?.addEventListener('click', async () => {
    try {
      ...
      for (let i = allNodes.length - 1; i >= 0; i--) {
        if (!allNodes[i].isRootCause && allNodes[i].parentId !== null) {
          targetNode = allNodes[i];
          break;
        }
      }
      if (!targetNode) {
        showToast('请先添加 Why 1 原因节点，再使用 AI 建议', 'warning');
        return;
      }
      await askAIForNode(targetNode.id);
    } finally {
      renderTree();
    }
  });
  ...
  document.getElementById('btnCloseSidebar')?.addEventListener('click', () => {
    ...
    // M3: 添加可选链，防止 DOM 结构变化时抛出 TypeError
  ...
  // Header logo -> reload (click + keyboard)
  ...
  // Step navigation event delegation (avoids inline onclick CSP issues in Tauri)
  ...
  const initializeApp = async () => {
    // Initialize IndexedDB first, then complete the bootstrap
    if (window.initDB) {
      await window.initDB();
    }
    ...
    // Auto-validate knowledge base on localhost (dev environment)
  ...
  const bootstrapApp = async function () {
    ...
    // M2: 改用 Promise.allSettled 替代 Promise.all，防止单个 plugin 加载失败导致应用白屏
    ...
    // (Sidebar → sidebar-nav.js, Tool Tabs → sidebar-nav.js)
    initSidebar();
    initStorageObserver();

    // Trigger initial list rendering and sidebar alerts
  ...
  const initSidebar = function () {
    ...
    // ── Expand / Collapse (pure toggle, no hover magic) ──
    ...
    // Toggle button: pin/unpin sidebar
  ...
  const initStorageObserver = function () {
    ...
    // 3. 监听树变更，debounce 回写问题 snapshot（Fix 1）
